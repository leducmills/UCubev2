...
public class UCubeV22 extends PApplet {
		...
		while (myPort.available () > 0) {
			...
			if (inString != null) {
				...
				if (firstContact == false) {
					//if we get a hello, clear the port, set firstcontact to false, send back an 'A', and print 'contact'
					...
					//inString = inString + "1000000010001000100010001000010010001000100110001001000010100000100001001000100010000000100010001000100010000100100010001001100010010000101000001000010010001000";
					...
					if (inString.equals(oldString)) {
						...
						for (int i = 0; i < inString.length(); i++) {
							...
							//throw away leading bit from each shift register (we're only using 7 of the 8 bits)
							if(i == 0 || i%8 == 0) { 
								inString.replace(bit, ' ');	
							}
							else {
								println("2: " + inString.length());

								//if the bit == 1, an led is plugged into that space, so look up it's coordinate
								if(bit == '1') {
									...
									if (knotVectors.contains(masterVectArray[counter])) {//not sure if this is right
									}
									else {
										knotVectors.add(masterVectArray[counter]);
										println(masterVectArray[counter] + " true");
										reDrawKnot = true;
									}
								}
								counter++;
		...
		for(int i = 0; i < 7; i++) {
			...
			for(int j = 0; j < 7; j++) {
				...
				for(int k = 0; k < 7; k++) {
					...
					Vec3D tempVect = new Vec3D(i * spacing, k * -spacing, j * spacing);  //maybe should be i,k,j so moves in Y
					Point3d tempPoint = new Point3d(i * spacing, k * -spacing, j * spacing);
	...
	public void hitDetection() {
		...
		for (int i = 0; i < vectors.length; i++) {
			...
			x = screenX((float)vectors[i].x, (float)vectors[i].y, (float)vectors[i].z);
			y = screenY((float)vectors[i].x, (float)vectors[i].y, (float)vectors[i].z);
			//println(x + " " + y);
